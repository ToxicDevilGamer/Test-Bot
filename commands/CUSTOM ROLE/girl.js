...
module.exports = {
    ...
    aliases : ["fri"],
    category : "customrole",
   description: "Add User To Girl Database & Role In Guild",
    run : async(client,message,args) => {
        ...
        if(!args[0]){
            return message.channel.send({embeds : [new MessageEmbed().setColor(`DARK_BUT_NOT_BLACK`).setDescription(`Usage : \`${prefix}\`girl <user>\``)]})
        }
